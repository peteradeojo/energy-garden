...
const GardenLink = ({ garden }) => {
  return (
    <Link className="flex bg-[#D9D9D9] hover:cursor-pointer hover:bg-gray-300 duration-200 rounded-md justify-between  items-center">
      <img src={checker} width={100} className="rounded-md" />
      <div className="grid gap-y-1">
        <p className="font-bold text-xl">{garden?.name || 'Garden name'}</p>
        <p>{garden?.plants.length || 12} plants</p>
      </div>
  ...
  return (
    <>
      ...
      <div className="flex gap-x-4 items-center">
        ...
        <button className="input-btn rounded-full w-[25px] h-[25px]">
          {'\u002b'}
        </button>
      </div>
